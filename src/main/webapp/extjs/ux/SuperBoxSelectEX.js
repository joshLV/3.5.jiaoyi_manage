 ...
 *
 */
Ext.ux.form.SuperBoxSelectEx = function(config) {
    Ext.ux.form.SuperBoxSelectEx.superclass.constructor.call(this, config);
    this.addEvents(
        /**
 ...
 */
Ext.ux.form.SuperBoxSelectEx = Ext.extend(Ext.ux.form.SuperBoxSelectEx, Ext.form.ComboBox, {
    /**
    ...
    /**
     * @cfg {String} change the allowBlank default to true
     */
    allowBlank: true,
    /*
     *[Dennis] :
    ...
    initComponent:function() {
        ...
        Ext.ux.form.SuperBoxSelectEx.superclass.initComponent.call(this);
    ...
    onRender:function(ct, position) {
        ...
        Ext.ux.form.SuperBoxSelectEx.superclass.onRender.call(this, ct, position);
        this.hiddenName = h;
        ...
        // 多一层DIV用于生成滚动条
        this.outerContentEl = this.outerWrapEl.wrap({
            tag : 'div',
            cls: 'x-superboxselect-content ' + extraClass
        });
        //设置高度属性
        this.outerContentEl.setHeight(this.height);
    ...
    onStoreLoad : function(store, records, options) {
        ...
        if (isValuesQuery) {
            ...
            if (this.mail_groups) {
                Ext.each(this.mail_groups, function(group) {
                    var showed = false;
                    
                    this.items.each(function(item) {
                        if (item.value == group.get('value')) {
                            showed = true;
                        };
                    });

                    if (!showed) {
                        this.addRecord(group);
                    };
                }, this);
            };
     ...
     * 更新控件 子项的编辑 状态
     */
    updateEditStateEx: function(){
    	var readOnly = this.readOnly;
    	var cls = 'x-superboxselect-btn-hide';
    	if(readOnly){
    		if(this.el){
    			this.el.un('keydown', this.onKeyDownHandler, this);
    		}
    		if(this.buttonExpand){
    			this.buttonExpand.addClass(cls);
    		}
    	}else{
    		if(this.buttonExpand){
    			this.buttonExpand.removeClass(cls);
    		}
    		if(this.el){
    ...
    initEvents : function() {
        ...
        Ext.ux.form.SuperBoxSelectEx.superclass.initEvents.call(this);
    ...
    onFocus: function() {
        ...
        Ext.ux.form.SuperBoxSelectEx.superclass.onFocus.call(this);
    },
    ...
    onBlur: function() {
        ...
        Ext.ux.form.SuperBoxSelectEx.superclass.onBlur.call(this);
    },
    ...
    onResize : function(w, h, rw, rh) {
        ...
        Ext.ux.form.SuperBoxSelectEx.superclass.onResize.call(this, w, h, rw, rh);
        this.autoSize();
    },
    onEnable: function() {
        Ext.ux.form.SuperBoxSelectEx.superclass.onEnable.call(this);
        this.items.each(function(item) {
            item.enable();
        });
        if (this.renderFieldBtns) {
    ...
    onDisable: function() {
        Ext.ux.form.SuperBoxSelectEx.superclass.onDisable.call(this);
        this.items.each(function(item) {
            item.disable();
        });
        if (this.renderFieldBtns) {
    ...
    clearValue : function(supressRemoveEvent) {
        Ext.ux.form.SuperBoxSelectEx.superclass.clearValue.call(this);
    ...
    reset :  function() {
        this.killItems();
        Ext.ux.form.SuperBoxSelectEx.superclass.reset.call(this);
        this.addedRecords = [];
    ...
    /**
     * Adds an array of items to the SuperBoxSelectEx component if the {@link #Ext.ux.form.SuperBoxSelect-allowAddNewData} config is set to true.
     * @methodOf Ext.ux.form.SuperBoxSelectEx
     * @name addItem
     * @param {Array} newItemObjects An Array of object literals containing the property names and values for an item. The property names must match those specified in {@link #Ext.ux.form.SuperBoxSelectEx-displayField}, {@link #Ext.ux.form.SuperBoxSelect-valueField} and {@link #Ext.ux.form.SuperBoxSelect-classField}
     */
    ...
    /**
     * Adds a new non-existing item to the SuperBoxSelectEx component if the {@link #Ext.ux.form.SuperBoxSelect-allowAddNewData} config is set to true.
    ...
    addItemBox : function(itemVal, itemDisplay, itemCaption, itemClass, itemStyle) {
        var hConfig, parseStyle = function(s) {
            ...
            return ret;
        }, itemKey = Ext.id(null, 'sbx-item'), box = new Ext.ux.form.SuperBoxSelectItemEx({
            owner: this,
    ...
    manageClearBtn : function() {
        ...
        var cls = 'x-superboxselect-btn-hide';
        if(this.buttonClear){
        	if (this.items.getCount() === 0 || this.readOnly) {
    ...
    initValue : function() {
        ...
        Ext.ux.form.SuperBoxSelectEx.superclass.initValue.call(this);
        if (this.mode === 'remote') {
    ...
    setValue : function(value) {
        ...
        if (this.mail_groups) {
            Ext.each(this.mail_groups, function(group) {
                var value_list = value.split(this.valueDelimiter);

                var need_show = false;
                for (var i=0;i<value_list.length;i++) {
                    if (value_list[i] == group.get('title')) {
                        need_show = true;
                        break;
                    };
                };
    ...
    onDestroy : function() {
        ...
        Ext.ux.form.SuperBoxSelectEx.superclass.onDestroy.call(this);
    },
    autoSize : function() {
        if (!this.rendered) {
            return this;
...
Ext.reg('superboxselectex', Ext.ux.form.SuperBoxSelectEx);
/*
 * @private
 */
Ext.ux.form.SuperBoxSelectItemEx = function(config) {
    Ext.apply(this, config);
    Ext.ux.form.SuperBoxSelectItemEx.superclass.constructor.call(this);
};
/*
 * @private
 */
Ext.ux.form.SuperBoxSelectItemEx = Ext.extend(Ext.ux.form.SuperBoxSelectItemEx, Ext.Component, {
    initComponent : function() {
        Ext.ux.form.SuperBoxSelectItemEx.superclass.initComponent.call(this);
    },
    ...
    //add by chenzh
    //update the item'S delete status
...
//    	}
    ...
    onRender : function(ct, position) {
        ...
        Ext.ux.form.SuperBoxSelectItemEx.superclass.onRender.call(this, ct, position);
    ...
    onDisable : function() {
        ...
        this.keyMap.disable();
        Ext.ux.form.SuperBoxSelectItemEx.superclass.onDisable.call(this);
    },
    onEnable : function() {
        if (this.hidden) {
        ...
        this.keyMap.enable();
        Ext.ux.form.SuperBoxSelectItemEx.superclass.onEnable.call(this);
    },
    onDestroy : function() {
        Ext.destroy(
                this.lnk,
        ...
        Ext.ux.form.SuperBoxSelectItemEx.superclass.onDestroy.call(this);
    }
});
